...
in
# stdenv (not stdenvNoCC): scriptc drives a C toolchain, and --dynamic builds
# the embedded JavaScript engine from the vendored sources in
# node_modules/@scriptc/runtime with CMake. Everything it compiles is vendored,
# so the build stays offline.
stdenv.mkDerivation {
  pname = "${packageJson.name}-scriptc";
  ...
  meta = {
    description = "${packageJson.description} (compiled with scriptc)";
    inherit (packageJson) homepage;
